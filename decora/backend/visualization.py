...
try:
    from .b_models import RoomInput, LayoutResult
except ImportError:
    from b_models import RoomInput, LayoutResult
...
def generate_floorplan(session_id: str, room: RoomInput, layout: LayoutResult) -> str:
    ...
    for f in layout.furniture:
        if f.name in _SKIP_IN_PLOT:
            continue
        plot_x, plot_y, plot_w, plot_h = f.x, f.y, f.w, f.h
        ax.add_patch(Rectangle((plot_x, plot_y), plot_w, plot_h,
                                facecolor=_FURNITURE_COLOR, edgecolor="black",
                                alpha=0.85, linewidth=1))
        ax.text(plot_x + plot_w / 2, plot_y + plot_h / 2, f.name.replace("_", " "),
